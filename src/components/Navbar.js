...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center max-w-[1545px] mx-auto md:px-12 px-4 p-2'>
      ...
      <div className={!nav ? 'fixed left-0 top-0 w-[50%] h-height-full bg-[#F8F8FF] ease-in-out duration-500 md:hidden z-10' : 'fixed left-[-100%] z-0'}>
        <Link className='flex items-center m-6'>
          <img className='size-16' src={Logo} alt="Logo AgroBoost" />
        </Link>
        <ul className='uppercase p-4'>
          ...
          <li className='p-2'>
            <Link to="/login" className='bg-custom-204E51 p-3 rounded-lg inline-block text-center w-width-full'>
              <span className='text-white lg:text-lg md:text-base sm:text-sm font-semibold'>Iniciar Sesión</span>
            </Link>
          </li>
          <li className='p-2'>
            <Link to="/registro" className='bg-white p-3 rounded-lg border-2 border-custom-264948 inline-block text-center w-width-full'>
              <span className='text-custom-264948 lg:text-lg md:text-base sm:text-sm font-semibold'>Registrarse</span>
            </Link>
          </li>
        </ul>
